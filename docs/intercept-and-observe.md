...
_⚠️ **警告**: 拦截器 和 监听器是低等级方法, 他们不应该被应用在正式的开发中。可以考虑使用 [reaction](reactions.md),因为 `observe` 不遵循事务原则也不支持对深度的对象变动的监听.使用这种方法是一种反面模式（anti-pattern）. 如果你想使用 `observe`来获取新值和旧的值,  请考虑优先使用[`reaction`](reactions.md#reaction)。 ⚠️_
